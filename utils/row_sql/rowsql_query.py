...
async def rowsql_update_attr_name(old_name: str, new_name: str, prefix: str, category_id: int):
    ...
    sql = '''UPDATE product SET attributes = change_value(attributes, '{"name": "%s"}', '%s') WHERE category_id = %s''' % (old_name, json_value_string, category_id)
    await conn.execute_query(sql)
    ...
    sql = '''UPDATE category SET filters = change_value(filters, '{"name": "%s"}', '%s') WHERE id = %s''' % (old_name, json_value_string, category_id)
    await conn.execute_query(sql)

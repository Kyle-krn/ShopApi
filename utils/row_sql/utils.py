...
async def generate_string_sql_query(category_attrs: List[CategoryFilters]):
    value_list_for_product = (
        []
    )  # Когда добавляется атрибут для всей категории в товары всегда улетает {"name": "name": "value": None}
    ...
    for attr in category_attrs:
        if type(attr.value) == list:
            ...
            if attr.value is True:
                ...
                value_string = "false"
        category_attr_sql_string = (
            "'" + '{"name": "%s", "value": %s, "prefix": "%s"}' % (attr.name, value_string, attr.prefix) + "'"
        )

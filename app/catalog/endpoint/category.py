...
from utils.row_sql.rowsql_query import (
    rowsql_add_attrs_for_all_category,
    rowsql_get_distinct_list_value,
    rowsql_remote_attrs_for_all_category,
    rowsql_update_attr_name,
    rowsql_get_distinct_string_value,
    rowsql_get_min_max_list_value,
    rowsql_get_min_max_price_discount,
)
...
async def get_users(active: int = 0, user: UserDB = Depends(current_active_user)):
...
@category_router.get(
    "/{category_id}",
    response_model_include={CatalogWithActiveProduct, GetCategory},
    responses={404: {"model": HTTPNotFoundError}},
)
async def get_category(category_id: int, active: bool = False, user: UserDB = Depends(current_active_user)):
    """Получить категорию по id"""
    category = await Category.get(id=category_id)
    if active:
        return CatalogWithActiveProduct(
            id=category.id,
            name=category.name,
            slug=category.slug,
            parent_id=category.parent_id,
...
async def get_control_category(
    category_id,
    name: bool = False,
    slug: bool = False,
    filters: bool = False,
...
async def get_list_string_attr_value(category_id: int, attr_name: str, user: UserDB = Depends(current_active_user)):
...
async def get_min_max_digit_attr_value(category_id: int, attr_name: str, user: UserDB = Depends(current_active_user)):
...
@category_router.patch(
    "/add_attr/{category_id}", response_model=GetCategory, responses={404: {"model": HTTPNotFoundError}}
)
async def add_attr_in_category(
    category_id: int, category_attrs: List[CategoryFilters], user: UserDB = Depends(current_superuser)
):
...
@category_router.patch(
    "/change_attr/{category_id}", response_model=GetCategory, responses={404: {"model": HTTPNotFoundError}}
)
async def change_attr(category_id: int, data_attr: List[ChangeNameAttr], user: UserDB = Depends(current_superuser)):
    ...
    for attr in category_attrs_for_sql:
        await rowsql_update_attr_name(
            old_name=attr.old_name, new_name=attr.new_name, prefix=attr.prefix, category_id=category_id
        )
    return await GetCategory.from_tortoise_orm(await Category.get(id=category_id))


@category_router.patch(
    "/remove_attr/{category_id}", response_model=GetCategory, responses={404: {"model": HTTPNotFoundError}}
)
async def remote_attr_in_category(
    category_id: int, remove_attrs: List[RemoveAttr], user: UserDB = Depends(current_superuser)
):

...
async def create_product(product: CreateProduct, user: UserDB = Depends(current_superuser)):
    """
    Если не передать в теле запроса attributes : [] и у категории есть фильтры, то атрибуты автоматически
    добавятся со значением None
    """
    ...
    try:
        ...
        if product.attributes:
            ...
            if len(product.attributes) < len(c_f):
                """
                Если какие то атрибуты не переданы в теле запроса,
                но есть в фильтрах категории то автоматически добавятся со значением null
                """
                for item in c_f:
...
@product_router.post(
    "/upload_photo/{product_id}", response_model=GetProduct, responses={404: {"model": HTTPNotFoundError}}
)
...
@product_router.post(
    "/query_by_category/{category_id}", response_model=PaginationProduct, responses={404: {"model": HTTPNotFoundError}}
)
async def query_products(
    ...
    return PaginationProduct(
        count=await Product.filter(q_query).count(), products=await GetProduct.from_queryset(queryset)
    )
...
@product_router.get(
    "/by_category/{category_id}", response_model=List[GetProduct], responses={404: {"model": HTTPNotFoundError}}
)
async def get_product_by_id(category_id: int, user: UserDB = Depends(current_active_user)):
...
async def get_product(product_id: int, user: UserDB = Depends(current_active_user)):
    return await GetProduct.from_tortoise_orm(await Product.get(id=product_id))


# Удалить фото
@product_router.delete(
    "/delete_photo/{product_id}", response_model=GetProduct, responses={404: {"model": HTTPNotFoundError}}
)
...
async def delete_product(product_id: int, user: UserDB = Depends(current_superuser)):
...
async def update_product(product_id: int, update_product: UpdateProduct, user: UserDB = Depends(current_superuser)):
    ...
    if update_product.attributes:
        for attr in update_product.attributes:
            ...
            if type(attr.value) != type(result[0]["value"]) and attr.value is not None:
                raise HTTPException(
                    status_code=404, detail=f"Атрибут {attr.name} ожидает тип {type(result[0]['value'])}."
                )

...
async def get_courier(city: str, user: UserDB = Depends(current_active_user)):
...
async def add_courier(courier: AddCourier, user: UserDB = Depends(current_superuser)):
...
async def get_pickup_point(pp_id: int, user: UserDB = Depends(current_active_user)):
...
async def get_pickup_point(city: str, user: UserDB = Depends(current_active_user)):
    """Поиск ПП по городу"""
...
async def add_pickup_point(pickup: AddPickUp, user: UserDB = Depends(current_superuser)):
    """Добавить ПП"""
    async with DadataAsync(DADATA_TOKEN, DADATA_SECRET) as dadata:
    ...
    await PickUpSettings.create(
        city=pickup.city.capitalize(),
